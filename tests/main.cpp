...
void TestBounds2() {
  ...
  assert(bigBoi.ContainsPoint(IPoint2(1, 2)) && !bigBoi.ContainsPoint(IPoint2(40, 50)));

  IBounds2 grower(IPoint2(-20, -10), IPoint2(20, 10));
  auto fattened = grower.Fatten(-0.5f);

  assert(fattened.lower == grower.lower / 2.f && fattened.upper == grower.upper / 2.f);
  assert(grower.Size() == 2 * fattened.Size());

  std::initializer_list<IPoint2> vertices = { { 12, 20 }, { 0, -5 }, { 100, 0 }, {-3, 35 } };
  IBounds2 vBounds = IBounds2(vertices);

  assert(vBounds.lower == IPoint2(-3, -5) && vBounds.upper == IPoint2(100, 35));
...
void TestRect2() {
  ...
  assert(bigBoi.Bounds().lower == bigBoi.origin && bigBoi.Bounds().upper == bigBoi.origin + bigBoi.size);
